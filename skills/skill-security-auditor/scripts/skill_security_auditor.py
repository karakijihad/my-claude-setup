...
# Windows consoles may use a legacy codepage (cp437/cp1252) — force UTF-8 so the
# emoji/box-drawing report output can't crash with UnicodeEncodeError.
...
CODE_PATTERNS = [
    ...
    # Windows credential harvesting / persistence — CRITICAL
    {
        "regex": r"USERPROFILE[^\n]{0,60}(?:\.ssh|\.aws|\.gnupg)|APPDATA[^\n]{0,60}(?:credentials|Cookies|Login Data)",
        "category": "CRED-HARVEST",
        "severity": Severity.CRITICAL,
        "risk": "Reads Windows credential locations (SSH/AWS dirs, browser credential stores)",
        "fix": "Remove all access to credential directories",
    },
    {
        "regex": r"(?:HKCU|HKLM|HKEY_CURRENT_USER|HKEY_LOCAL_MACHINE)[^\n]{0,80}\\+Run\b|\breg(?:\.exe)?\s+add\b",
        "category": "PRIV-ESC",
        "severity": Severity.CRITICAL,
        "risk": "Windows registry Run-key manipulation — persistence mechanism",
        "fix": "Remove registry autorun modifications. Skills should not install persistence",
    },
    {
        "regex": r"\bschtasks\b[^\n]*/create|\bNew-ScheduledTask\b",
        "category": "PRIV-ESC",
        "severity": Severity.CRITICAL,
        "risk": "Windows scheduled task creation — persistence mechanism",
        "fix": "Remove scheduled task creation. Skills should not modify scheduled tasks",
    },
    {
        "regex": r"profile\.ps1|Start Menu[^\n]{0,40}Startup",
        "category": "FS-ABUSE",
        "severity": Severity.CRITICAL,
        "risk": "Writes to PowerShell profile or Windows Startup folder — persistence mechanism",
        "fix": "Remove all writes to PowerShell profiles and Startup folders",
    },
    ...
    {
        # World-writable modes only (last octal digit grants write: 2,3,6,7) or o+w/a+w.
        # Deliberately does NOT match chmod 755 / 0o755 — those are normal and safe.
        "regex": r"\bchmod\b[^\n]*\b0?o?[0-7]{2}[2367]\b|\bchmod\b[^\n]*\b[ao]\+w\b",
        "category": "PRIV-ESC",
        "severity": Severity.HIGH,
        "risk": "Setting world-writable permissions",
    ...
    {
        # unsafe_load always flags; yaml.load flags only when no SafeLoader appears in the call.
        "regex": r"\byaml\.unsafe_load\s*\(|\byaml\.load\s*\((?![^)]*SafeLoader)",
        "category": "DESERIAL",
        "severity": Severity.HIGH,
...
def scan_file_prompt_injection(filepath: Path, report: AuditReport):
    """Scan a markdown file for prompt injection AND embedded shell attack patterns.

    SKILL.md instructions are executable in practice — an agent following a
    `curl … | bash` line in markdown runs it. See threat-model.md "Pattern 3".
    """
    try:
        content = filepath.read_text(encoding="utf-8", errors="replace")
    except Exception:
    ...
    for i, line in enumerate(lines, 1):
        for pat in list(PROMPT_INJECTION_PATTERNS) + list(SHELL_PATTERNS):
            if re.search(pat["regex"], line):
                report.findings.append(
                    Finding(
                        severity=pat["severity"],
