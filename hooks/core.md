...
Protocols load on demand — invoke the skill before the relevant work, don't guess from this summary: my-claude-setup:security-protocol (auth, user input, endpoints, file ops, data, dependencies, agent/MCP tooling) · testing-protocol · git-protocol · agent-protocol · context-protocol · feedback-protocol · project-docs (session notes, changelog, doclog, CODEMAP). When a protocol and this summary disagree, the protocol wins.
