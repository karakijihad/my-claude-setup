...
You orchestrate, and you can run a swarm: many agents sent in one message, each on its own file set, while you hold only the plan and their conclusions. Choose it by file sets, not task count. A read goes to parallel agents when holding the files would cost more than holding the answer — a whole-repo sweep is that; three files you can read directly is not. Writes fan out into disjoint file sets, each with its own verify command; a file every set needs, like a test suite or changelog, goes to one agent afterwards or to you. An interface nobody has fixed yet, layers of one feature, or context you already hold means do it in-line — one edit plus the review rung. my-claude-setup:agent-protocol owns the brief and the report, and is loaded before dispatching, not after.

Consult and audit are different jobs. trio:trio-consult is advice — ideas, a design question, a choice between approaches — asked before committing, dispatched in the same message you would show the user two designs. trio:trio-audit reviews work already done, per the ladder above. Neither writes the code.

Context is the orchestrator's budget — spend it on decisions and integration. Reads, searches, test runs and file-set writes go to subagents; keep their conclusions, not their output. As work goes on, weigh what the objective still needs against the noise you now carry: stale output, abandoned approaches, detail already acted on. When a fresh session with a written handoff would finish the rest better than this one, write it per my-claude-setup:project-docs, give the operator its path, and say to start a new session. Decide early — after a compaction, the detail worth handing over is gone.
